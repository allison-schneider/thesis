...
def initial_position(lat,lon):
	"""Choose the initial position. Transforms longitude from [-180,180) to [0,360).
...
def destination(lat1, lon1, distance, bearing):
    ...
    bearing -- Direction value between 0 and 360 degrees, clockwise from true North.
    """
    earth_radius = 6371e3
    angular_distance = distance / earth_radius
    lat2 = np.degrees(np.arcsin(np.sin(np.radians(lat1)) * np.cos(angular_distance)
                            + np.cos(np.radians(lat1)) * np.sin(angular_distance) * np.cos(np.radians(bearing))))
...
def next_position(position_lat, position_lon, filename):
	...
	new_lat, new_lon = destination(grid_lat, grid_lon, displacement, wind_bearing)
	return new_lat, new_lon

# Get trajectory
def trajectory():
	trajectory = np.zeros((81,2))
	current_lat, current_lon = initial_position(42.3603088, -71.0893148)
...
def globe_plot():
	trajectory_lat, trajectory_lon = trajectory()
...
def equidistant_plot():
	...
	# draw the edge of the map projection region (the projection limb)
	map.drawmapboundary(fill_color='aqua')
	map.plot(trajectory_lon, trajectory_lat, latlon=True, zorder=1)
	plt.show()	
